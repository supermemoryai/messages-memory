...
interface ChatAreaProps {
  ...
  onBack?: () => void;
  onSendMessage: (message: string, conversationId?: string, attachments?: Attachment[]) => void;
...
export function ChatArea({
  ...
  const isReactionActive = (message: any, type: ReactionType) => {
    return (
      message.reactions?.some((r: Reaction) => r.type === type && r.sender === "me") ??
      false
    );
  };
    ...
    <div className="h-dvh relative bg-background">
      ...
      >
        {activeConversation?.messages.map((message, index) => {
          ...
          const nextMessage = index < activeConversation.messages.length - 1 ? activeConversation.messages[index + 1] : null;
            ...
            >
              <div className={cn("flex flex-col relative", isMe ? "items-end" : "items-start")}>
                ...
                {message.attachments && message.attachments.length > 0 && (
                  <div className={cn("flex gap-2 flex-wrap mb-1 max-w-[280px] sm:max-w-md", isMe ? "justify-end" : "justify-start")}>
                      ...
                      {Object.entries(menuReactionIcons).map(([type, icon]) => (
                        <button
                          ...
                          className={cn(
                            ...
                            isReactionActive(message, type as ReactionType)
                      ...
                      {[...message.reactions]
                        ...
                        .map((reaction, index, array) => (
                          <div
                            ...
                            style={{
                              ...getReactionStyle(reaction, isMe, isMobileView ?? false),
