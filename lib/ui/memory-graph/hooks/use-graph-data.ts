...
export function useGraphData(
	...
	return useMemo(() => {
		...
		documentNodes.forEach((docNode) => {
			...
			doc.memoryEntries.forEach((memory: MemoryEntry, memIndex: number) => {
		...
		// Add version-chain edges (old -> new) using relatedMemories
		data.documents.forEach((doc) => {
			doc.memoryEntries.forEach((mem: MemoryEntry) => {
				// Use the new relatedMemories field with derives/extends/updates
				if (mem.relatedMemories && mem.relatedMemories.length > 0) {
					mem.relatedMemories.forEach((relation) => {
						const fromId = memNodeIdMap.get(relation.relatedMemoryId);
						...
						if (fromId && toId) {
							allEdges.push({
								...
								color: colors.relations[relation.relationType] ?? colors.relations.updates,
								edgeType: "version",
				...
				// Legacy support for old memoryRelations field
				else if (
					mem.memoryRelations &&
					typeof mem.memoryRelations === "object" &&
					Object.keys(mem.memoryRelations).length > 0
				) {
					const parentRelations = mem.memoryRelations as unknown as Record<string, MemoryRelation>;
				...
				// Legacy support for parentMemoryId field
				else if (mem.parentMemoryId) {
					const fromId = memNodeIdMap.get(mem.parentMemoryId);
					const toId = memNodeIdMap.get(mem.id);
					if (fromId && toId) {
						allEdges.push({
							id: `version-${fromId}-${toId}`,
							...
							color: colors.relations.updates,
							edgeType: "version",
							relationType: "updates" as MemoryRelation,
						});
					}
				}
			});
